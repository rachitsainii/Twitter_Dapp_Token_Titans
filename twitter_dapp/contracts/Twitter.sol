...
contract Twitter{
    ...
    struct tweet{
        address tweeter;  // Address
        uint256 id;       // Id
        string tweetText; // Contents of the tweet
        string tweetImg;  // Optional Image file
        bool isDeleted;   // Delete the tweet
        uint256 timestamp;  // Show the date of the tweet
    }
    ...
    event tweetCreated(   
        ...
        bool isDeleted,
        uint256 timestamp
    );  // This event will occur in the add tweet function.
    ...
    function getAllTweets() public view returns (tweet[] memory){
        tweet[] memory temporary = new tweet[](counter);  // Empty array of counter length
        uint countTweets = 0;

        for(uint i=0; i<counter; i++){     // Using this for loop we make a list of all the tweets which are not deleted
